...
void handlerChange(){
  ...
  sighandler_t sig = signal(2,&setFlag1);
  if(sig!=(void*)SIG_DFL){
    printf(2,"Original handler wasn't SIG_DFL, test failed\n");
    return;
  }
  sig = signal(2,&setFlag1);
  if(sig!=&setFlag1){
  ...
  sig = signal(4,&setFlag1);
  if(fork()==0){
    if(signal(3,(void*)SIG_IGN)!=(void*)SIG_IGN || signal(4,&setFlag1)!=&setFlag1){
      printf(2,"Signal handlers changed after fork. test failed\n");
      return;
    }
    char* argv[] = {"signalTest","sighandlers",0};
    if(exec(argv[0],argv)<0){
        printf(2,"couldn't exec, test failed\n");
        exit();
    }
...
void multipleChildrenTest(){
  ...
  printf(2,"multiple children test passed\n");
}
